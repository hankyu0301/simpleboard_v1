package org.hankyu.simpleboard_v1.helper;



import org.hankyu.simpleboard_v1.exception.CannotConvertNestedStructureException;
...
public class NestedConvertHelper<K, E, D> {
    ...
    private List<D> convertInternal() {
        ...
        //entities는 사전조건 ( repository.findAllOrderByParentIdAscNullsFirstCategoryIdAsc() )에 의해 정렬된 상태
        for (E e : entities) {
            D dto = toDto(e); //c -> new CategoryDto(c.getId(), c.getName(), new ArrayList<>())
            map.put(getKey(e), dto);
            if (hasParent(e)) {
                E parent = getParent(e); // List<D> roots 조회.
